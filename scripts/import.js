...
async function run() {
  ...
  try {
    ...
    await page.goto('https://halsail.com', { waitUntil: 'networkidle' });
    ...
    if (!importBtn) throw new Error('Could not find Upload button');
    ...
    const confirmedRows = await page.$$eval('table tbody tr', (trs) =>
      trs.map((tr) => {
        ...
        return { helm: cells[0]||'', className: cells[1]||'', sailNo: cells[2]||'', crew: cells[3]||'' };
    ...
    await sendNotify(CF_WORKER_URL, { type: 'error', message: `❌ WSC import failed: ${err.message}` }).catch(()=>{});
    process.exit(1);
  } finally {
    await browser.close();
  }
